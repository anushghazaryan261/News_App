...
class NewsRepo {
    ...
    suspend fun categoryNews(category: String): List<ArticleResponse> {
        return RetrofitHelper.getInstance().create(NewsApiService::class.java).categoryNews(category = category).run {
            this.body()?.articles?.map {
                ArticleResponse(it.source?: Source("",""), it.urlToImage?: "", it.title?: "", it.author?:"", it.description?:"")
            } ?: listOf()
        }
    ...
    suspend fun searchNews(searchString: String): List<ArticleResponse> {
        return RetrofitHelper.getInstance().create(NewsApiService::class.java).searchNews(searchString = searchString).run {
            this.body()?.articles?.map {
                ArticleResponse(it.source?: Source("",""), it.urlToImage?: "", it.title?: "", it.author?:"", it.description?:"")
            } ?: listOf()
        }
